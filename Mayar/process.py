...
class Processing(soundFile.SoundFile):
    ...
    def extractFeatures(self):
        ...
        try:
            self.centroid_feature = librosa.feature.spectral_centroid(
                self.data, self.samplingFrequency)
            self.rolloff_feature = librosa.feature.spectral_rolloff(
                self.data, self.samplingFrequency)
            self.mfcc_feature = librosa.feature.mfcc(
                self.data, self.samplingFrequency)
            self.chroma_stft_feature = librosa.feature.chroma_stft(
                self.data, self.samplingFrequency)
        except:
            print("enter data")
    ...
    def Hash(self):
        print("Hash")
        try:
            features = [self.centroid_feature, self.rolloff_feature,
                        self.mfcc_feature, self.chroma_stft_feature]
