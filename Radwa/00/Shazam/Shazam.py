...
class ApplicationWindow(QtWidgets.QMainWindow):
    ...
    def readsignal(self, songnum):
        self.fname = QtWidgets.QFileDialog.getOpenFileName(
            self, ' Open File', os.getenv('home'), "mp3(*.mp3) ;; wav(*.wav)")
        self.path = self.fname[0]
        if songnum == 0:
            print(self.results)
            self.paths[0] = self.path
            self.ui.name_song1.setText(os.path.splitext(
                os.path.basename(self.path))[0])  # to write song name
    ...
    def findsimilarto(self, feat1, feat2, feat3):
        ...
        for i in range(len(data)):
            ...
            for j in range(1, 4):
                ...
                diffvalue[j] = (1-(imagehash.hex_to_hash(str(newfeats[j])) -
                                   imagehash.hex_to_hash(str(songfeats[j]))) / 256.0)
            print('NEW FEAT', newfeats)
            print('SONG FEAT', songfeats)
            similarity = (diffvalue[1]+diffvalue[2]+diffvalue[3])/0.03
            similarity1=diffvalue[3]
        ...
        for i in range(10):
            self.ui.tableWidget.setItem(
                i, 0, QtWidgets.QTableWidgetItem(self.results[i][0]))
            self.ui.tableWidget.setItem(
                i, 1, QtWidgets.QTableWidgetItem(str((self.results[i][1]))+"%"))
    ...
    def similarity(self):
        if (self.paths[0] is not None) & (self.paths[1] is not None):
            ...
            self.mixedsong = self.song11.mix(self.song22, self.slider)
            self.findsimilarto(self.mixedsong.feature1(
            ), self.mixedsong.feature2(), self.mixedsong.feature3())
        elif (self.paths[0] is not None):
            self.thissong1 = song(self.paths[0])
            self.findsimilarto(self.thissong1.feature1(
            ), self.thissong1.feature2(), self.thissong1.feature3())
        elif (self.paths[1] is not None):
            self.thissong2 = song(self.paths[1])
            self.findsimilarto(self.thissong2.feature1(
            ), self.thissong2.feature2(), self.thissong2.feature3())
        else:
            print("no songs")
